...
#' }
#'
recurrent = function(df, Primary, Secondary, End_survey, survey_duration = 10) {
  data = df %>%
    mutate_if(is.factor, as.character) %>%
    filter(Species %in% c(Primary, Secondary) & DateTime <= End_survey) %>%
    group_by(Site) %>%
    arrange(DateTime, .by_group = TRUE) %>%

    # ID of the primary event (Site + Primary event number)
    mutate(PredObs = ifelse(Species %in% Primary, 1, 0) %>% cumsum(),
  ...
  data = left_join(data, df, join_by(Site == Site, DateTime == DateTime), relationship = "many-to-many")
  data %<>% select(-Species) %>%
